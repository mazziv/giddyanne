...
from src.api import create_app as create_http_app
...
from src.startup import create_components, run_indexing, start_watcher
...
def get_pid_file_path(storage_dir: Path, host: str, port: int) -> Path:
    ...
    return storage_dir / f"{host_part}-{port_part}.pid"


def get_log_file_path(storage_dir: Path, host: str, port: int) -> Path:
    """Get the log file path. Omit host/port from name when using defaults."""
    host_part = "" if host == DEFAULT_HOST else host
    port_part = "" if port == DEFAULT_PORT else str(port)
    return storage_dir / f"{host_part}-{port_part}.log"


def find_running_server(storage_dir: Path) -> tuple[str, int, int] | None:
    """Find a running server by scanning pid files. Returns (host, port, pid) or None."""
    if not storage_dir.exists():
        return None

    for pid_file in storage_dir.glob("*-*.pid"):
        try:
...
def stop_server(storage_dir: Path) -> bool:
    """Stop running server. Returns True if stopped, False if not running."""
    result = find_running_server(storage_dir)
    if result is None:
        return False
    host, port, pid = result
    pid_path = get_pid_file_path(storage_dir, host, port)
    try:
...
async def main(
    root_path: Path, project_config: ProjectConfig, storage_dir: Path,
    ...
    log_path = get_log_file_path(storage_dir, host, port)
    ...
    async def background_indexing():
        ...
        c.stats.record_startup_complete()
...
if __name__ == "__main__":
    ...
    existing = find_running_server(storage_dir)
    if existing:
    ...
    try:
        loop.run_until_complete(
            main(root_path, project_config, storage_dir, host, port, args.verbose)
        )
