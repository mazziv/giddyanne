...
Works out of the box in any git repo. For better results, add a `.giddyanne.yaml` config with path descriptions — see [Configuration](config.md) or run `giddy init`.
...
A: Initial indexing uses CPU for embedding generation. After that, the search server is lightweight and only re-indexes changed files. If you run multiple projects, a shared embedding server loads the model once (~700MB) and all project servers share it (~180MB each).

## License

MIT License - see [LICENSE.txt](https://github.com/upship-ai/giddyanne/blob/main/LICENSE.txt)

## Contributing

Contributions welcome! See [Contributing](contributing.md) for development setup and areas where we'd love help.
